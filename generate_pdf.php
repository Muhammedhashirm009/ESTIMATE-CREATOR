...
// Create PDF
$pdf = new EstimatePDF($shopName, $shopAddress, $shopPhone, $shopEmail, $shopWebsite, $shopGST, $shopLogo, $bankName, $accountNumber, $ifscCode, $accountHolder);
$pdf->AddPage();
...
$pdf->Cell(35,10,"Rate (Rs.)",1,0,'C', true);
...
for ($i = 0; $i < count($items); $i++) {
    if (!empty($items[$i]) && !empty($quantities[$i]) && !empty($prices[$i])) {
        ...
        // Calculate if description needs multiple lines
        $charWidth = $pdf->GetStringWidth('A'); // Approximate character width
        $maxChars = floor(70 / $charWidth); // Max chars that fit in 70 width
        $descLines = ceil(strlen($description) / $maxChars);
        $rowHeight = max(10, $descLines * 5 + 2); // Dynamic height based on text
        
        // Store current position
        $x = $pdf->GetX();
        $y = $pdf->GetY();
        
        // Draw all cells with uniform height
        $pdf->Cell(10,$rowHeight,$itemCount,1,0,'C');
        $pdf->Cell(70,$rowHeight,'',1,0,'L'); // Empty bordered cell for description
        $pdf->Cell(25,$rowHeight,number_format($quantity, 0),1,0,'C');
        $pdf->Cell(35,$rowHeight,number_format($price, 2),1,0,'R');
        $pdf->Cell(40,$rowHeight,number_format($amount, 2),1,0,'R');
        
        // Now add the description text inside the bordered cell
        $pdf->SetXY($x + 11, $y + 2); // Position inside the description cell
        $pdf->MultiCell(68, 5, $description, 0, 'L');
...
// Add empty rows if needed (minimum 3 total rows)
while ($itemCount <= 3) {
...
// Totals section - keep it simple and centered
$pdf->Ln(5);
$pdf->SetFont('Arial','B',12);
...
// Output PDF
$filename = 'estimate_' . preg_replace('/[^a-zA-Z0-9]/', '_', $customerName) . '_' . date('Ymd') . '.pdf';
